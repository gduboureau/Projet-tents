...
bool test_game_equal(void){
  ...
  for(uint i = 0;i<DEFAULT_SIZE;i++){
    for(uint j = 0;j<DEFAULT_SIZE;j++){
      if ((game_get_square(g1,i,j) != game_get_square(g2,i,j) || game_get_expected_nb_tents_col(g1,j)!=game_get_expected_nb_tents_col(g2,j) || game_get_expected_nb_tents_row(g1,i)!=game_get_expected_nb_tents_row(g2,i)) && game_equal(g1,g2) == true){
        game_delete(g1);
        game_delete(g2);
        game_delete(g3);
        game_delete(g4);
      ...
      if (((game_get_square(g1,i,j) == game_get_square(g2,i,j)) && game_get_expected_nb_tents_col(g1,j)==game_get_expected_nb_tents_col(g2,j) && game_get_expected_nb_tents_row(g1,i)!=game_get_expected_nb_tents_row(g2,i)) && game_equal(g3,g4) == false){
        game_delete(g1);
        game_delete(g2);
        game_delete(g3);
        game_delete(g4);
