...
bool test_game_is_over(void){
    ...
    for(uint i=0;i<DEFAULT_SIZE;i++){
        for(uint j=0;j<DEFAULT_SIZE;j++){ 
            ...
            if (i==0 && j>0 && j<DEFAULT_SIZE-1 && game_get_square(g,i,j)== TENT && (game_get_square(g,i+1,j) != TREE || game_get_square(g,i,j-1)!=TREE || game_get_square(g,i,j+1) != TREE)){
                game_delete(g);
                return false;
            }
            if(i>0 && i<DEFAULT_SIZE-1 && j>0 && j<DEFAULT_SIZE-1 && game_get_square(g,i,j)== TENT && (game_get_square(g,i-1,j)!=TREE || game_get_square(g,i+1,j)!= TREE || game_get_square(g,i,j-1)!= TREE || game_get_square(g,i,j+1)!= TREE)){
                game_delete(g);
                return false;
            }
            if(i==0 && j==0 && game_get_square(g,i,j)== TENT &&( game_get_square(g,i+1,j) != TREE || game_get_square(g,i,j+1) != TREE)){
                game_delete(g);
                return false;
            }
            if (i>0 && j==0 && i<DEFAULT_SIZE-1 && game_get_square(g,i,j)== TENT && (game_get_square(g,i+1,j) != TREE || game_get_square(g,i-1,j)!=TREE || game_get_square(g,i,j+1) != TREE)){
                game_delete(g);
                return true;
            }
            if (i==DEFAULT_SIZE-1 && j==0 && game_get_square(g,i,j)== TENT && (game_get_square(g,i-1,j) != TREE || game_get_square(g,i,j+1)!=TREE)){
                game_delete(g);
                return true;
            }
            if (i==DEFAULT_SIZE-1 && j>0 && j<DEFAULT_SIZE-1  && game_get_square(g,i,j)== TENT && (game_get_square(g,i-1,j) != TREE || game_get_square(g,i,j-1)!=TREE || game_get_square(g,i,j+1) != TREE)){
                game_delete(g);
                return true;
            }
            if (i==DEFAULT_SIZE-1 && j==DEFAULT_SIZE-1 && game_get_square(g,i,j)== TENT && (game_get_square(g,i-1,j) != TREE || game_get_square(g,i,j-1)!=TREE)){
                game_delete(g);
                return true;
            }
            if (i>0 && j==DEFAULT_SIZE-1 && i<DEFAULT_SIZE-1  && game_get_square(g,i,j)== TENT && (game_get_square(g,i-1,j) != TREE || game_get_square(g,i+1,j)!=TREE || game_get_square(g,i,j-1) != TREE)){
                game_delete(g);
                return true;
            }
            if (i==0 && j==DEFAULT_SIZE-1 && game_get_square(g,i,j)== TENT && (game_get_square(g,i-1,j) != TREE || game_get_square(g,i,j-1)!=TREE)){
                game_delete(g);
                return true;
            }
            }
